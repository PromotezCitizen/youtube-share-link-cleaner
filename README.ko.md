...
YouTube 공유 링크의 `si`를 제거하고, 복사한 YouTube 시청 링크를 필요할 때 짧게 정리하는 Chrome 확장 프로그램입니다.
...
- 복사한 `youtube.com/watch?v=…` 링크를 필요할 때 `youtu.be/…`로 짧게 바꿉니다.
- 현재 열어 둔 YouTube 페이지의 정리된 링크를 팝업에서 바로 복사합니다.
- 일반 YouTube 동영상 페이지(Shorts 제외)에 **정리해 복사** 버튼을 추가합니다.
...
1. Chrome 주소창에서 YouTube 링크를 복사합니다.
2. 확장 프로그램 버튼에서 **YouTube Share Link Cleaner**를 엽니다.
3. **복사한 링크 정리**를 누릅니다.

지원하는 `youtube.com/watch?v=VIDEO_ID` URL은 `https://youtu.be/VIDEO_ID`로 바뀌며, `si`는 제거하고 `t`, `list` 등의 나머지 파라미터는 유지합니다. 클립보드는 버튼을 눌렀을 때만 읽으며 자동으로 감시하지 않습니다.

## 현재 YouTube 페이지 링크 복사

YouTube 페이지를 보고 있을 때 확장 프로그램을 열고 **현재 페이지 링크 복사**를 누릅니다. 현재 URL에서 `si`를 제거하고, 지원하는 시청 URL은 `youtu.be/VIDEO_ID`로 짧게 바꿔 복사합니다. YouTube가 아닌 탭에서는 아무것도 복사하지 않습니다.

## 동영상 페이지에서 바로 복사

일반 YouTube 동영상 페이지에서는 YouTube의 동작 버튼 옆에 **정리해 복사** 버튼이 추가됩니다. 팝업을 열지 않고 눌러 현재 링크의 `si`를 제거하고 짧게 만든 뒤 복사할 수 있습니다. YouTube Shorts 페이지에는 이 버튼이 표시되지 않습니다.
...
- **복사한 링크 정리**를 누를 때만 복사된 URL을 읽고 기기 안에서 처리한 뒤 정리된 값으로 즉시 바꿉니다. 해당 값은 보관하거나 전송하지 않습니다.
...
- `storage`: 자동 정리 토글 상태를 현재 기기에 저장합니다.
- `clipboardRead`: **복사한 링크 정리**를 누른 직후에만 복사된 텍스트를 읽습니다.
- `clipboardWrite`: 정리된 YouTube 링크로 복사한 텍스트를 바꿉니다.
- `tabs`: **현재 페이지 링크 복사**를 누른 뒤에만 활성 탭 URL을 읽어 요청한 정리 링크를 만듭니다.
