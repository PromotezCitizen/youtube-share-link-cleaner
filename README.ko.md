...
YouTube 공유 링크의 `si`를 제거하고, 원하면 Shorts 링크도 짧게 만드는 Chrome 확장 프로그램입니다.
...
- 확장 프로그램 팝업에서 `si` 제거를 즉시 켜거나 끌 수 있습니다.
...
- `chrome.storage.local`에는 `si` 제거와 선택적인 Shorts·라이브 줄이기 설정만 저장됩니다.
...
이 확장 프로그램이 요청하는 권한은 `storage` 하나입니다. `si` 제거와 선택적인 Shorts·라이브 줄이기 설정을 현재 기기에 저장하는 용도로만 사용합니다.
